...
Dit hoofdstuk gaat kort in op de mogelijkheden om het 
migratieproces
...
Waarom kan momenteel niet het volledige migratieproces geautomatiseerd
...
\begin{itemize}
    ...
    \item vertalen van business-namen naar technische namen die de 
    mainframe kent
    \item optimalisaties die de toolset zelf doet van flows
\end{itemize}

Wat AI en automatisering wel kunnen ondersteunen is het vertalen van
standaard CA Gen-constructen naar procedurele PL/I-voorzetten. In de
praktijk blijven semantische keuzes, datamodelrelaties en host-specifieke
naamgeving nog steeds verantwoordelijkheden voor menselijke experts.
...
ondersteunen, maar geen volledige conversie afdwingen. Deze workflow kan
...
\begin{enumerate}
    ...
    \item \textbf{Regelgebaseerde mapping en AI-vertaling}: eenvoudige
    constructen worden door regelgebaseerde mappings omgezet; AI helpt bij
    complexere vertalingen, commentaar en boilerplate SQL.
    \item \textbf{Review en validatie door experts}: een CA Gen/PL/I-expert
    controleert de output, valideert naamgeving en datamodelrelaties, en
    voert een parallel run uit of vergelijkt functionele output.
\end{enumerate}
...
code. Belangrijke invoer voor AI zijn onder meer:
\begin{itemize}
...
voorbeeld wordt een stukje pseudo-code uit een Action Block genomen
...
Er zitten heel wat verschillen tussen de AI-gegenereerde code en de code van de expert. Het grootste verschil dat direct opvalt is de naamgeving van alle variabelen. Dit is ook logisch want developpers programmeren in de CA GEN-toolset met businessnamen die CA Gen automatisch aan de hand van de host-encyclopedie omzet naar technische namen die de mainframe wel kent. AI kan deze vertalingen niet maken omdat AI geen toegang heeft tot de host-encyclopedie. Ook deelt de expert logica op over verschillende procedures, vooral SQL-blokken, om code leesbaarder te maken.

In de praktijk betekent dit dat AI vooral geschikt is voor de eerste vertaalslag: syntactische omzetting, standaard SQL-templates en boilerplate-code. De uiteindelijke beoordeling moet altijd door een expert gebeuren, met specifieke aandacht voor datamodel-integriteit, host-naamgeving en foutafhandeling.
