...
de theoretische achtergrond behandelt, en hoofdstuk~\ref{ch:methodologie} 
de gevolgde onderzoeksaanpak beschrijft, wordt het framework hier opgebouwd rond vier kerncomponenten: 
(1)~een gestructureerde \emph{analysefase}, (2)~een set \emph{vertaalrichtlijnen}, 
...
\begin{itemize}
    ...
    \item \textbf{Type applicaties:} batch\- en procedure\-georiënteerde modules, sequentiële bestandsoutput en/of DB2-interacties.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Vertaalrichtlijnen} (\autoref{sec:component-vertaling}) een referentietabel van CA~Gen\-constructen en hun PL/I\-equivalenten, voortbouwend op de syntactische 
...
\begin{table}[h]
\centering
\caption[Schema migratieframework]{High-level schema van het migratieframework voor CA~Gen naar PL/I.}
\label{tab:framework-schema}
\small
\begin{tabular}{|p{0.18\linewidth}|p{0.34\linewidth}|p{0.34\linewidth}|}
\hline
\textbf{Stap} & \textbf{Doel / output} & \textbf{Terugkoppeling / validatie} \\
\hline
Analysefase & Modelontsluiting, View-inventaris, Action-flow, USE-graaf, datamatrix & Basis voor vertaling en validatie \\
\hline
Vertaalrichtlijnen & Referentietabel CA~Gen $\rightarrow$ PL/I, datatype- en view-mapping, SQL-strategie & Feedback uit de PoC, review en analyse \\
\hline
Best practices & Paradigmaverschuiving, PLAPO-conventies, expliciete view matching, exitstate-mapping & Voorkomt herhaalde migratiefouten \\
\hline
Validatie & Parallel Run, outputvergelijking, functionele equivalentie & Verifieert migratie en identificeert verbeterpunten \\
\hline
\end{tabular}
\end{table}

Figuur~\ref{fig:framework} benadrukt dat het framework niet alleen een lineair stappenplan is, maar ook iteratief werkt: bevindingen uit Component~4 kunnen teruggevoerd worden naar Component~1 en leiden tot een toegenomen betrouwbaarheid van de migratie.
...
uit \autoref{sec:Views} (Imports, Exports, Locals, Entity Actions) wordt hierbij gebruikt als 
...
\hline
\textit{(naam)} & Import / Export / Local & Entity / Work / Group & \textit{(max.\ kardinaliteit, \dots)} \\
\hline
\end{tabular}
\end{table}

\subsection{Stap 1.3 USE\-graaf en modulegrenzen}
...
\begin{itemize}
    ...
    \item Type: Common Action Block (CAB, intern) of External Action Block (EAB bestaat reeds in 
    PL/I of COBOL).
...
\subsection{Stap 1.4 Database\- en interface\-afhankelijkheden}
...
en worden hier niet herhaald; deze sectie biedt de \emph{referentietabel} 
(\autoref{tab:vertaaltabel}) en bespreekt de aandachtspunten per categorie.

\subsection{Mappingtabel}
\label{subsec:mappingtabel}

\begin{table}[h]
\centering
\caption[Vertaaltabel CA~Gen $\rightarrow$ PL/I]{Vertaaltabel van CA~Gen\-constructen naar PL/I.}
...
\hline
\texttt{CASE OF} & \texttt{SELECT;} \dots \texttt{WHEN \dots} \texttt{OTHER \dots} \texttt{END;} & \\
\hline
...
met een afgesproken set codes (bv.\ \texttt{0}~=~OK, \texttt{4}~=~waarschuwing, \texttt{8}~=~fout).
...
(\autocite{Orosz2022}, \autocite{Patel2024}).
...
Zorg dat in de JCL waarmee de main-module aangeroepen wordt de juiste IMS-database tabellen meegegeven worden.
...
\subsection{Stap 4.1 -- Parallel uitvoeren}
\label{subsec:parallel-run}

De twee implementaties draaien een tijdje parallel en na iedere uitvoering wordt gekeken naar eventuele verschillen tussen de output-bestanden om alle edge-cases eruit te halen.

%\subsection{Stap 4.2 -- Expert-review}
...
\section{\IfLanguageName{dutch}{Toepassing op de PoC}{Application to the PoC}}%
\label{sec:framework-poc}

Het framework is parallel met dit hoofdstuk toegepast op de migratie van de CA~Gen\-module 
\texttt{AB\_APP\_GBP\_MAT\_BAF} naar PL/I. De analysefase wordt uitgewerkt in Hoofdstuk~\ref{ch:reverse-engineering}, de
uiteindelijke implementatie in Hoofdstuk~\ref{ch:poc}, en de validatie in Hoofdstuk~\ref{ch:validatie}.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{framework-visueel.png}
    \caption[Visuele voorstelling van het framework]{Visuele voorstelling van het framework.}
    \label{fig:framework}
\end{figure}
