...
\begin{table}[h]
\centering
\caption[Schema reverse engineering]{Schema van de reverse engineering-analyse voor \texttt{AB\_APP\_GBP\_MAT\_BAF}.}
\label{tab:re-schema}
\small
\begin{tabular}{|p{0.22\linewidth}|p{0.32\linewidth}|p{0.35\linewidth}|}
\hline
\textbf{Stap} & \textbf{Uitvoering} & \textbf{Resultaat} \\
\hline
Encyclopedie ontsluiten & Check Out van het relevante businessmodel, versiebeheer en lokale toegang tot CA~Gen & Lokale modelkopie, versiehistoriek, traceerbaarheid \\
\hline
View-inventaris & Inventaris per Action Block volgens Imports, Exports, Locals, Entity Actions & Overzicht van alle views en hun rol in de module \\
\hline
USE-graaf & USE-relaties & USE-graaf \\
\hline
Datamatrix & Mapping van READ/READ EACH/CREATE/UPDATE/DELETE naar DB2-tabellen & Datamatrix voor databaseafhankelijkheden en validatie \\
\hline
Bevindingen & Conclusies uit de analysefase voor de PL/I-implementatie & Analysecontract met risico's, aandachtspunten en consistentie-afspraken \\
\hline
\end{tabular}
\end{table}
...
Een uittreksel van de inventaris is opgenomen in bijlage~\ref{ch:bijlage-views}.
...
Het aanroepen van welke Action Block's van \texttt{AB\_APP\_GBP\_MAT\_BAF} werd ontleed. De hoofdlus itereert over
...
\end{itemize}
De volledige USE-graaf is opgenomen in het analysedossier (zie
...
overzicht. 
%De gedetailleerde versie inclusief WHERE-clausules en sorteervoorwaarden zit in
%Bijlage~\ref{ch:bijlage-views}.
...
\begin{enumerate}
    ...
    \item De gehanteerde exitstate-codes wijken licht af van de standaardconventie binnen andere
        modules; in de PL/I-implementatie wordt voor consistentie de standaardconventie gebruikt.
\end{enumerate}
